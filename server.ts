...
// Process a list of TON events into our normalized transaction shape (extracted so
// it can be reused by the main wallet endpoint AND the paginated "load more" endpoint
// and the single-transaction detail endpoint)
async function processTonEvents(allEvents: any[], myAddress: string, currentNftsMap: Map<string, any>): Promise<any[]> {
  ...
  return enrichNftTransfersWithMetadata(processedTransactions as any[], currentNftsMap);
}

// API Routes

// Returns the client's IP address — used by the frontend to namespace
// per-visitor favorites/search history in localStorage
...
async function fetchWalletData(network: string, address: string, cacheKey: string): Promise<any> {
  try {
    ...
    if (network === "ton") {
      ...
      const enrichedTransactions = await processTonEvents(allEvents, myAddress, currentNftsMap);
      ...
      data = {
        ...
        nextBeforeLt: allEvents.length === 100 ? allEvents[allEvents.length - 1].lt : null,
        nfts: normalizedNfts,
        stats: {
      ...
      const [ltcRes, price] = await Promise.all([
        axios.get(`https://api.blockcypher.com/v1/ltc/main/addrs/${address}/full?limit=50`, { timeout: 10000 }),
        getCachedPrice("litecoin")
      ]);

      const balance = (ltcRes.data.balance / 1e8).toFixed(8);

      data = {
        address,
        balance: `${balance} LTC`,
        usdValue: parseFloat(balance) * price,
        transactions: (ltcRes.data.txs || []).slice(0, 50).map((tx: any) => {
          const isIncoming = (tx.outputs || []).some((o: any) => (o.addresses || []).includes(address));
          const totalOut = (tx.outputs || []).reduce((sum: number, o: any) => sum + (o.value || 0), 0);
          return {
            hash: tx.hash,
            date: tx.confirmed || new Date().toISOString(),
            from: tx.inputs?.[0]?.addresses?.[0] || "Multiple Inputs",
            to: tx.outputs?.[0]?.addresses?.[0] || "Multiple Outputs",
            amount: (totalOut / 1e8).toFixed(8) + " LTC",
            status: tx.confirmations > 0 ? "Success" : "Pending",
            fee: ((tx.fees || 0) / 1e8).toFixed(8) + " LTC",
            direction: isIncoming ? 'in' : 'out',
            type: 'Transfer'
          };
        }),
        stats: {
          totalReceived: ( (ltcRes.data?.total_received || 0) / 1e8).toFixed(8) + " LTC",
          totalSent: ( (ltcRes.data?.total_sent || 0) / 1e8).toFixed(8) + " LTC",
          txCount: ltcRes.data?.n_tx || 0,
          firstTx: "N/A",
          lastTx: "N/A",
          maxBalance: ( (ltcRes.data?.total_received || 0) / 1e8).toFixed(8) + " LTC (Est.)"
        },
        tokens: []
      };
...
// Load more (older) transactions — used by the "More" button in transaction history
app.get("/api/transactions/ton/:address", async (req, res) => {
  const { address } = req.params;
  const beforeLt = req.query.before_lt as string | undefined;
  if (!beforeLt) return res.status(400).json({ error: "before_lt is required" });

  const cacheKey = `tx_page_ton_${address}_${beforeLt}`;
  const cached = getCache(cacheKey);
  if (cached) return res.json(cached);

  try {
    const result = await dedupeRequest(cacheKey, async () => {
      const [accountRes, eventsRes, nftsRes] = await Promise.all([
        axios.get(`https://tonapi.io/v2/accounts/${address}`, { timeout: 8000, headers: TONAPI_HEADERS }),
        axios.get(`https://tonapi.io/v2/accounts/${address}/events?limit=100&before_lt=${beforeLt}`, { timeout: 8000, headers: TONAPI_HEADERS }),
        dedupeRequest(`nfts_ton_${address}_inflight`, () =>
          axios.get(`https://tonapi.io/v2/accounts/${address}/nfts?limit=100`, { timeout: 8000, headers: TONAPI_HEADERS })
        )
      ]);

      const myAddress = accountRes.data.address;
      const events: any[] = eventsRes.data.events || [];

      const currentNftsMap = new Map();
      for (const nft of (nftsRes.data?.nft_items || [])) {
        if (nft?.address) {
          currentNftsMap.set(nft.address, nft);
          currentNftsMap.set(formatTonAddress(nft.address), nft);
        }
      ...
      const transactions = await processTonEvents(events, myAddress, currentNftsMap);
      return {
        transactions,
        nextBeforeLt: events.length === 100 ? events[events.length - 1].lt : null
      };
    });
    ...
    const status = error.response?.status || 500;
    res.status(status).json({ error: error.message || "Failed to fetch transactions" });
  }
});

// Jetton (token) balances — lazy-loaded by the Tokens tab
app.get("/api/jettons/ton/:address", async (req, res) => {
  const { address } = req.params;
  const cacheKey = `jettons_ton_${address}`;
  const cached = getCache(cacheKey);
  if (cached) return res.json(cached);

  try {
    const jettonsRes = await dedupeRequest(cacheKey + "_inflight", () =>
      axios.get(`https://tonapi.io/v2/accounts/${address}/jettons?currencies=usd`, { timeout: 8000, headers: TONAPI_HEADERS })
    );

    const tokens = (jettonsRes.data?.balances || [])
      .map((b: any) => {
        const decimals = b.jetton?.decimals ?? 9;
        const balance = parseInt(b.balance || "0") / Math.pow(10, decimals);
        const usdPrice = parseFloat(b.price?.prices?.USD || "0");
        return {
          name: b.jetton?.name || "Unknown Token",
          symbol: b.jetton?.symbol || "TOKEN",
          image: b.jetton?.image,
          balance: balance.toFixed(decimals > 4 ? 4 : decimals),
          rawBalance: balance,
          usdValue: balance * usdPrice,
          verified: b.jetton?.verification === "whitelist",
          address: formatTonAddress(b.jetton?.address || "")
        };
      })
      .filter((t: any) => t.rawBalance > 0)
...
// Single transaction detail — shows the full breakdown of an event, including
// (for NFT transfers) the ownership history "tree" of that NFT, similar to tonviewer.
app.get("/api/tx/ton/:hash", async (req, res) => {
  const { hash } = req.params;
  const cacheKey = `tx_detail_ton_${hash}`;
  const cached = getCache(cacheKey);
  if (cached) return res.json(cached);

  try {
    const result = await dedupeRequest(cacheKey, async () => {
      const eventRes = await axios.get(`https://tonapi.io/v2/events/${hash}`, { timeout: 8000, headers: TONAPI_HEADERS });
      const event = eventRes.data;

      // Process this single event without a "my address" perspective (use the
      // sender of the primary action as the reference point for direction).
      const primaryAction =
        event.actions?.find((a: any) => a.type === "NftItemTransfer") ||
        event.actions?.find((a: any) => a.type === "JettonTransfer") ||
        event.actions?.find((a: any) => a.type === "TonTransfer") ||
      ...
      const referenceAddress =
        primaryAction?.TonTransfer?.recipient?.address ||
        primaryAction?.JettonTransfer?.recipient?.address ||
        primaryAction?.NftItemTransfer?.recipient?.address ||
        "";

      const [tx] = await processTonEvents([event], referenceAddress, new Map());

      let nftOwnershipHistory: any[] | null = null;
      let jettonFlow: any | null = null;

      if (tx?.nftInfo?.address) {
        // NFT items are themselves accounts on TON — their event history is the ownership history
        try {
          const nftEventsRes = await axios.get(`https://tonapi.io/v2/accounts/${primaryAction.NftItemTransfer.nft}/events?limit=50`, { timeout: 8000, headers: TONAPI_HEADERS });
          nftOwnershipHistory = (nftEventsRes.data?.events || [])
            .flatMap((e: any) => e.actions.filter((a: any) => a.type === "NftItemTransfer"))
            .map((a: any) => ({
              from: formatTonAddress(a.NftItemTransfer.sender?.address || "N/A"),
              fromName: a.NftItemTransfer.sender?.name,
              to: formatTonAddress(a.NftItemTransfer.recipient?.address || "N/A"),
              toName: a.NftItemTransfer.recipient?.name,
              comment: a.NftItemTransfer.comment
            }))
            .reverse();
        } catch (e: any) {
          console.error("NFT history fetch failed:", e.message);
        }
      }

      if (primaryAction?.type === "JettonTransfer") {
        const j = primaryAction.JettonTransfer;
        jettonFlow = {
          jetton: { name: j.jetton?.name, symbol: j.jetton?.symbol, image: j.jetton?.image, address: formatTonAddress(j.jetton?.address || "") },
          from: formatTonAddress(j.sender?.address || "N/A"),
          fromName: j.sender?.name,
          to: formatTonAddress(j.recipient?.address || "N/A"),
          toName: j.recipient?.name,
      ...
      return {
        ...tx,
        opCode: primaryAction?.NftItemTransfer ? "Nft Ownership Assigned · 0x05138d91" :
                primaryAction?.JettonTransfer ? "Jetton Transfer" :
                primaryAction?.TonTransfer ? "TON Transfer" : tx?.type,
        nftOwnershipHistory,
        jettonFlow,
        rawEvent: event
      };
    ...
    const status = error.response?.status || 500;
    res.status(status).json({ error: error.message || "Failed to fetch transaction" });
  }
});
